async function fetchData() {
  try {
    const response = await fetch(
      window.moneyNutUrl('/family/annual_view/annual_get_target'),
    );
    if (!response.ok) {
...
async function fetchExpectedIncomeData() {
  const response = await fetch(
    window.moneyNutUrl('/family/annual_view/annual_Expected_income/'),
  );
  const Data = await response.json();
  return Data;
}
...
async function fetchExpectedSavingData() {
  const response = await fetch(
    window.moneyNutUrl('/family/annual_view/annual_Expected_Saving/'),
  );
  const Data = await response.json();
  return Data;
}
