async function fetchData() {
  const reponse = await fetch(window.moneyNutUrl('/saving/saving_target_get'), {
    method: 'GET',
  });
...
async function fetchfreq() {
  const reponse = await fetch(window.moneyNutUrl('/saving/freq_get'), {
    method: 'GET',
  });
...
async function saving_tagert_update(newValue, saving_target_id) {
  const response = await fetch(
    window.moneyNutUrl('/saving/saving_target_update'),
    {
      method: 'PUT',
      headers: {
        'X-CSRFToken': getCookie('moneynut_csrftoken'),
